# Find the smallest number in the tree

class Node:
    def __init__(self, val, left=None, right=None):
        self.val =  val
        self.left = left
        self.right = right

### SOLUTION ###
# Complexity: n = number of nodes
# Time: 0(n)
# Space: 0(n)

# Using BFS
def treeMin(root):
    if not root: return 0
    smallest = float('inf') # Setting the base number to infinity
    queue = [root]
    while len(queue) > 0:
        current = queue.pop(0)
        if current.val < smallest: smallest = current.val
        if current.left: queue.append(current.left)
        if current.right: queue.append(current.right)
    return smallest

# Using DFS - Recursive
def treeMinRecursive(root):
    if not root: return float('inf')
    leftMin = treeMinRecursive(root.left)
    rightMin = treeMinRecursive(root.right)
    return min(root.val, leftMin, rightMin)

################

if __name__ == '__main__':

    ''' Construct the following tree
              1
            /   \
           /     \
          2       3
           \     / \
            4   5   6
               / \
              7   8
    '''
    
root = Node(1)
root.left = Node(2)
root.right = Node(3)
root.left.right = Node(4)
root.right.left = Node(5)
root.right.right = Node(6)
root.right.left.left = Node(7)
root.right.left.right = Node(8)

node = root.right.left.left
print(treeMin(root)) # Expected Result: 1
print(treeMinRecursive(root)) # Expected Result: 1
